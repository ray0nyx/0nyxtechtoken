...
-- Drop any existing auto_confirm_user function and trigger
DROP TRIGGER IF EXISTS auto_confirm_user ON auth.users;
DROP FUNCTION IF EXISTS auth.auto_confirm_user();

-- Create a new auto_confirm_user function that uses the proper method
...
AS $function$
BEGIN
  -- Use the proper method to confirm users
  -- This avoids directly updating the confirmed_at column
  PERFORM auth.confirm_user(NEW.id);
  RETURN NEW;
END;
$function$;
