-- Ensure master_traders table exists
...
-- Ensure master_trader_followers table exists
...
-- Ensure copy_trading_settings table exists
CREATE TABLE IF NOT EXISTS copy_trading_settings (
  ...
  position_size_percentage DECIMAL(5, 2) DEFAULT 10,
...
DROP POLICY IF EXISTS "Anyone can view master traders" ON master_traders;
DROP POLICY IF EXISTS "Service role can manage master traders" ON master_traders;
DROP POLICY IF EXISTS "Authenticated users can insert master traders" ON master_traders;
DROP POLICY IF EXISTS "Users can view their own follows" ON master_trader_followers;
DROP POLICY IF EXISTS "Users can follow master traders" ON master_trader_followers;
DROP POLICY IF EXISTS "Users can update their own follows" ON master_trader_followers;
DROP POLICY IF EXISTS "Users can unfollow master traders" ON master_trader_followers;
DROP POLICY IF EXISTS "Users can view their own copy settings" ON copy_trading_settings;
DROP POLICY IF EXISTS "Users can manage their own copy settings" ON copy_trading_settings;

-- RLS Policies for master_traders (public read, authenticated can insert/update)
CREATE POLICY "Anyone can view master traders"
  ON master_traders FOR SELECT
  USING (true);

CREATE POLICY "Authenticated users can insert master traders"
  ON master_traders FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can update master traders"
  ON master_traders FOR UPDATE
...
CREATE POLICY "Users can manage their own copy settings"
  ON copy_trading_settings FOR ALL
  USING (auth.uid() = user_id);
...
DROP TRIGGER IF EXISTS update_follower_count_trigger ON master_trader_followers;
...
-- Drop trigger if exists and recreate
DROP TRIGGER IF EXISTS update_master_traders_timestamp ON master_traders;
