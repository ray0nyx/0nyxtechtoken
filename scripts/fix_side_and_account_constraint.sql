...
-- AND ensure proper account handling to prevent foreign key violations
...
-- Function to ensure an account exists for a user
...
BEGIN
  ...
  IF v_account_id IS NULL THEN
    v_account_id := ensure_account_exists(p_user_id, 'Tradovate Account');
  ELSE
