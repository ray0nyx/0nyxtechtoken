...
class MarketDataService:
    ...
    async def get_ohlcv(
        ...
        async with self.rate_limiter:
            # Try Birdeye for Solana addresses (32-44 chars) or specific override
            if (len(symbol) > 30 and "/" not in symbol) or exchange == "birdeye":
                try:
                    data = await self._get_ohlcv_birdeye(symbol, timeframe, limit)
                    if data:
                        return data
                except Exception as e:
    ...
    async def _get_ohlcv_birdeye(
        ...
        try:
            ...
            # Estimate seconds per candle for time_from calculation
            seconds_map = {
                "1m": 60, "5m": 300, "15m": 900, "30m": 1800,
                "1h": 3600, "4h": 14400, "1d": 86400, 
                "1w": 604800, "1M": 2592000
            }
            candle_seconds = seconds_map.get(timeframe, 3600)
            
            now = datetime.utcnow()
            time_to = int(now.timestamp())
            time_from = time_to - (limit * candle_seconds)
                ...
                if resp.status == 200:
                    ...
                    for c in candles:
                        ohlcv_list.append(OHLCVData(
                            timestamp=datetime.fromtimestamp(c["unixTime"]),
                            open=float(c["o"]),
                            high=float(c["h"]),
                            low=float(c["l"]),
                            close=float(c["c"]),
            ...
            print(f"Birdeye logic failed for {symbol}: {e}")
            return []
