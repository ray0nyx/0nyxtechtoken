...
-- This script is specifically designed for Supabase's PostgreSQL environment
-- Run this in your Supabase SQL Editor or via the Supabase CLI

-- Note: TimescaleDB extension should already be enabled in Supabase
-- If not, you may need to request it to be enabled by Supabase support
...
BEGIN
    ...
    -- Test getting data
    IF EXISTS (SELECT 1 FROM market_data WHERE symbol = 'BTC/USD' LIMIT 1) THEN
        RAISE NOTICE 'TimescaleDB setup test completed successfully!';
    ELSE
        RAISE NOTICE 'TimescaleDB setup completed, but test data insertion failed.';
    END IF;
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'TimescaleDB setup completed with warnings: %', SQLERRM;
END $$;
