-- Fix for Tradovate CSV timestamp parsing issues
-- This ensures that entry_date and exit_date values from the CSV are properly used
...
BEGIN
  ...
  FOR i IN 0..v_total_rows - 1 LOOP
    ...
    BEGIN
      ...
      -- Get the entry_date and exit_date directly from the JSON
      -- IMPORTANT: These values are already properly formatted in the frontend
      v_entry_date_str := COALESCE(
        v_trade->>'entry_date',
        NULL
      );
      
      v_exit_date_str := COALESCE(
        v_trade->>'exit_date',
        NULL
      );
      
      -- Extract timestamps from metadata or direct fields (as fallback)
      ...
      -- PRIORITY 1: Use the pre-formatted entry_date and exit_date from the JSON
      -- These have already been properly parsed in the frontend
      BEGIN
        -- For entry date
        IF v_entry_date_str IS NOT NULL THEN
          v_entry_date := v_entry_date_str::TIMESTAMP;
          v_debug_log := array_append(v_debug_log, format('Using pre-formatted entry_date: %s', v_entry_date_str));
        END IF;
        
        -- For exit date  
        IF v_exit_date_str IS NOT NULL THEN
          v_exit_date := v_exit_date_str::TIMESTAMP;
          v_debug_log := array_append(v_debug_log, format('Using pre-formatted exit_date: %s', v_exit_date_str));
        END IF;
      EXCEPTION WHEN OTHERS THEN
        v_debug_log := array_append(v_debug_log, format('Error parsing pre-formatted dates: %s', SQLERRM));
      END;
      
      -- PRIORITY 2: Fall back to timestamps if the pre-formatted dates fail
      -- Parse timestamps more flexibly - attempt multiple formats
      IF v_entry_date IS NULL AND v_bought_timestamp IS NOT NULL THEN
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
          v_entry_date := NOW();
        END;
      ELSIF v_entry_date IS NULL THEN
        v_entry_date := NOW();
      END IF;
      
      -- For exit date (only if not already set)
      IF v_exit_date IS NULL AND v_sold_timestamp IS NOT NULL THEN
        BEGIN
          -- Try multiple timestamp formats in order of likelihood
          -- Try ISO format
          BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
          v_exit_date := COALESCE(v_entry_date, NOW());
        END;
      ELSIF v_exit_date IS NULL THEN  
        v_exit_date := COALESCE(v_entry_date, NOW());
      END IF;
      ...
      BEGIN
        ...
        -- Second priority: Use exit_date
        IF v_date IS NULL AND v_exit_date IS NOT NULL THEN
          v_date := v_exit_date::DATE;
        END IF;
        
        -- Third priority: Use entry_date
      ...
      v_debug_log := array_append(v_debug_log, format('Row %s: Inserting trade for symbol=%s, side=%s, qty=%s, entry=%s, exit=%s, pnl=%s, date=%s, entry_date=%s, exit_date=%s', 
        i, v_symbol, v_side, v_quantity, v_entry_price, v_exit_price, v_pnl, v_date, v_entry_date, v_exit_date));
      ...
      INSERT INTO trades (
        ...
        v_pnl,
        v_fees,
        v_entry_date,    -- Using the pre-formatted entry_date value
        v_exit_date,     -- Using the pre-formatted exit_date value
        ...
        jsonb_build_object(
          ...
          'debug_info', jsonb_build_object(
            'entry_date_source', CASE 
              WHEN v_entry_date_str IS NOT NULL THEN 'json_entry_date_field'
              WHEN v_bought_timestamp IS NOT NULL THEN 'timestamp_parsing'
              ELSE 'default_current_time'
            END,
            'exit_date_source', CASE 
              WHEN v_exit_date_str IS NOT NULL THEN 'json_exit_date_field'
              WHEN v_sold_timestamp IS NOT NULL THEN 'timestamp_parsing'
              ELSE 'default_current_time'
            END
          )
        )
...
-- Fix function to update any existing trades with wrong dates
CREATE OR REPLACE FUNCTION fix_tradovate_entry_exit_dates()
RETURNS TEXT AS $$
DECLARE
  v_count INTEGER := 0;
  v_error TEXT;
BEGIN
  -- Update trades using metadata timestamps
  UPDATE trades t
  SET 
    entry_date = 
      CASE 
        -- Try to get the entry_date from metadata boughtTimestamp
    ...
    exit_date = 
      CASE 
        -- Try to get the exit_date from metadata soldTimestamp
  ...
  WHERE 
    ...
    AND t.extended_data->'original_data' ? 'metadata';
    
  -- Get count of rows affected
  GET DIAGNOSTICS v_count = ROW_COUNT;
  
  RETURN format('Fixed %s Tradovate trades entry and exit dates', v_count);
EXCEPTION WHEN OTHERS THEN
  GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
...
-- Run the fix function to update existing trades
SELECT fix_tradovate_entry_exit_dates();

-- Revoke and grant execute permissions
DO $$
BEGIN
  EXECUTE 'REVOKE EXECUTE ON FUNCTION process_tradovate_csv_batch(UUID, JSONB, UUID) FROM PUBLIC';
  EXECUTE 'GRANT EXECUTE ON FUNCTION process_tradovate_csv_batch(UUID, JSONB, UUID) TO authenticated';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Error setting permissions: %', SQLERRM;
END $$; 