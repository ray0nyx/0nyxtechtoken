-- Direct fix for dollar sign handling in CSV import
...
-- Create a function to handle missing Fill Time
CREATE OR REPLACE FUNCTION handle_tradovate_missing_fill_time(v_row JSONB)
RETURNS JSONB AS $$
DECLARE
  v_fill_time TEXT;
BEGIN
  -- Check if Fill Time is missing
  IF v_row->>'Fill Time' IS NULL OR v_row->>'Fill Time' = '' THEN
    -- Try to use boughtTimestamp or soldTimestamp if available
    IF v_row->>'boughtTimestamp' IS NOT NULL AND v_row->>'boughtTimestamp' != '' THEN
      v_fill_time := v_row->>'boughtTimestamp';
    ELSIF v_row->>'soldTimestamp' IS NOT NULL AND v_row->>'soldTimestamp' != '' THEN
      v_fill_time := v_row->>'soldTimestamp';
    ELSE
      -- If no timestamps are available, use current timestamp
      v_fill_time := NOW()::TEXT;
    END IF;
    
    -- Add Fill Time to the row
    v_row := jsonb_set(v_row, '{Fill Time}', to_jsonb(v_fill_time));
  END IF;
...
BEGIN
  ...
  IF p_account_id IS NULL THEN
    ...
    IF v_account_id IS NULL THEN
      ...
      VALUES (p_user_id, 'Default Tradovate Account', 'Tradovate', 'DEFAULT', FALSE, TRUE)
