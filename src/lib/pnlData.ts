...
// Helper function to format a date to YYYY-MM-DD in local time zone
...
export async function getTodayPnL(): Promise<{ trades: TradeData[]; totalPnL: number }> {
  ...
  const todayTrades = trades.filter(trade => {
    const isMatch = isTradeDateMatch(trade, todayStr);
    console.log('getTodayPnL: Trade date check:', {
      tradeDate: trade.date || trade.entry_date || trade.exit_date,
      tradeDateFormatted: trade.date || trade.entry_date || trade.exit_date ? formatDateToLocalISOString(new Date(trade.date || trade.entry_date || trade.exit_date)) : 'N/A',
      todayStr,
      isMatch,
      pnl: trade.pnl
    });
