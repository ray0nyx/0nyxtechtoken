...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM migration_log WHERE migration_name = '20240715000018_fix_analytics_data'
  ) THEN
    -- Log migration
    INSERT INTO migration_log (migration_name, applied_at) 
    VALUES ('20240715000018_fix_analytics_data', NOW());
    ...
    CREATE TABLE analytics (
        ...
        average_pnl DECIMAL DEFAULT 0,
        wins INTEGER DEFAULT 0,
        losses INTEGER DEFAULT 0,
        largest_win DECIMAL DEFAULT 0,
        largest_loss DECIMAL DEFAULT 0,
        daily_pnl JSONB DEFAULT '{}'::jsonb,
        weekly_pnl JSONB DEFAULT '{}'::jsonb,
        monthly_pnl JSONB DEFAULT '{}'::jsonb,
        cumulative_pnl DECIMAL DEFAULT 0,
    ...
    BEGIN
        ...
        SELECT
            total_trades,
            ROUND(CAST(COALESCE(total_pnl, 0) as NUMERIC), 2),
            ROUND(CAST(CASE 
            ...
            ROUND(CAST(CASE 
                WHEN total_trades > 0 THEN 
                    COALESCE(total_pnl, 0) / total_trades 
                ELSE 0 
            END as NUMERIC), 2),
            COALESCE(wins, 0),
            COALESCE(losses, 0),
    ...
    RAISE NOTICE 'Migration 20240715000018_fix_analytics_data has been applied';
  END IF;
END $$; 