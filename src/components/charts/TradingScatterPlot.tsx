...
interface TradingScatterPlotProps {
  trades: any[];
  showCard?: boolean;
}

export function TradingScatterPlot({ trades = [], showCard = true }: TradingScatterPlotProps) {
  const processTradeData = (trades: any[]) => {
    ...
    return sortedTrades.map((trade) => {
      ...
      return {
        x: date.getHours() + (date.getMinutes() / 60), // x-axis: time of day
        y: pnl, // y-axis: PnL
        isWin,
  ...
  const totalPnL = data.reduce((sum: number, trade: any) => sum + trade.y, 0);
  ...
  return (
    <Card className="border-slate-700/50 dark:border-slate-700/50 border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:shadow-orange-500/10 overflow-hidden bg-slate-900 dark:bg-slate-900">
      <CardHeader className="bg-orange-500/10 dark:bg-orange-500/20 pb-3">
        <div className="flex justify-between items-center">
          <CardTitle className="text-lg font-medium text-white dark:text-slate-900 flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-orange-500/20 dark:bg-orange-500/30">
              <Clock className="h-4 w-4 text-orange-400 dark:text-orange-600" />
            </div>
            P&L Time of Day
          </CardTitle>
