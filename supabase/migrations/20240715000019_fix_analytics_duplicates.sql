...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM migration_log WHERE migration_name = '20240715000019_fix_analytics_duplicates'
  ) THEN
    -- Log migration
    INSERT INTO migration_log (migration_name, applied_at) 
    VALUES ('20240715000019_fix_analytics_duplicates', NOW());
        ...
        SELECT
            ...
            ROUND(CAST(CASE 
                WHEN total_trades > 0 THEN 
                    (COALESCE(wins, 0)::DECIMAL / total_trades) * 100 
                ELSE 0 
            END as NUMERIC), 2),
            ROUND(CAST(CASE 
                WHEN total_trades > 0 THEN 
    ...
    RAISE NOTICE 'Migration 20240715000019_fix_analytics_duplicates has been applied';
  END IF;
END $$; 