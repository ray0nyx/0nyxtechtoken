-- Migration: Add Tradovate CSV functions
-- Description: Adds functions to handle Tradovate CSV imports and populate analytics tables

-- Log this migration
DO $$
BEGIN
  INSERT INTO public.migration_log (migration_name, description, executed_at)
  VALUES ('20240712000001_add_tradovate_csv_functions', 'Adds functions to handle Tradovate CSV imports and populate analytics tables', NOW());
EXCEPTION
  WHEN OTHERS THEN
...
DROP TRIGGER IF EXISTS process_tradovate_csv_data_trigger ON trades;
DROP TRIGGER IF EXISTS tradovate_analytics_update ON trades;

-- Drop existing Tradovate-related functions to avoid conflicts
DROP FUNCTION IF EXISTS clean_dollar_sign(TEXT) CASCADE;
DROP FUNCTION IF EXISTS handle_tradovate_missing_fill_time(JSONB) CASCADE;
...
DROP FUNCTION IF EXISTS populate_tradovate_analytics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_tradovate_analytics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_tradovate_detailed_analytics(UUID) CASCADE;
DROP FUNCTION IF EXISTS process_tradovate_csv_data() CASCADE;
DROP FUNCTION IF EXISTS update_tradovate_analytics_on_trade_change() CASCADE;
